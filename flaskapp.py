...
def artistquery(artist_id):
    """
    Displays all tracks for a specific artist.
    ...
    rows = execute_query(
        ...
        WHERE ArtistId = %s
        """,
        (artist_id,)   # tuple required for parameterized query
    )
...
def time_form_post():
    """
    POST handler: reads the milliseconds entered by the user,
    then calls viewtime() to run the query.
    """
    text = request.form['text']
